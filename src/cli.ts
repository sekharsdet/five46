...
async function runE2eTest(
  ...
  const provider = getLlmProvider(llmProvider || 'openai')
  ...
  writeFileSync(outPath, redactSecrets(generateAgentSpec(run), secrets), 'utf8')
...
const USAGE = [
  'Usage: five46 test <url> --goal "text" [--max-steps N] [--headed] [--out path] [--storage-state path] [--allow-deletes] [--no-root-cause]',
  '       five46 login <url> --goal "text" --out <path> [--max-steps N] [--headed]',
  '       five46 api <base-url> --goal "text" [--allow-writes] [--allow-deletes] [--allow-host <host>]... [--storage-state path] [--max-steps N] [--out path] [--no-root-cause]',
...
async function main() {
  ...
  if (argv[0] === 'login') {
    const { url, goal, maxSteps, headed, out } = parseAgentArgs(argv.slice(1))
    if (!url || !goal || !out) {
      console.error(USAGE)
      process.exit(1)
    }
    ...
    try {
      await runLoginFlow(url, goal, maxSteps, headed, out)
    } catch (err) {
  ...
  if (argv[0] === 'api') {
    const { baseUrl, goal, maxSteps, out, storageState, allowWrites, allowDeletes, allowHosts, noRootCause } = parseApiArgs(argv.slice(1))
    if (!baseUrl || !goal) {
      console.error(USAGE)
      process.exit(1)
    }
    ...
    try {
      await runApiTestCommand(baseUrl, goal, maxSteps, out, storageState, allowWrites, allowDeletes, allowHosts, noRootCause)
    } catch (err) {
  ...
  const { url, goal, maxSteps, headed, out, storageState, allowDeletes, noRootCause } = parseAgentArgs(argv.slice(1))
  if (!url || !goal) {
    console.error(USAGE)
    process.exit(1)
  }
  ...
  try {
    await runE2eTest(url, goal, maxSteps, headed, out, storageState, allowDeletes, noRootCause)
  } catch (err) {
